...
levelToList = A.elems . board

instance Hashable Cell where
	hashWithSalt :: Int -> Cell -> Int
	hashWithSalt x (Cell curType) = hashWithSalt x curType

instance Hashable Level where
	hashWithSalt :: Int -> Level -> Int
	hashWithSalt x lvl = hashWithSalt x $ levelToList lvl 
{-
	*** TODO ***
