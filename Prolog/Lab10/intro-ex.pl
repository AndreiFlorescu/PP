...
myConcat([],L,L).
myConcat([Head | Tail1], L, [Head | Tail2]) :- myConcat(Tail1, L, Tail2).
...
myReverse([],[]).
myReverse([Head | Tail], RevList) :- myReverse(Tail, Sol),
                                     myConcat(Sol, [Head], RevList).
...
myReverseAcc([Head | Tail], Acc, RevList) :- myReverseAcc(Tail, [Head | Acc], RevList).
...
extract([Head | Tail], 0, End, [Head | Tail2]) :- End1 is End - 1,
                                                  extract(Tail, 0, End1, Tail2).
extract([_ | Tail], Start, End, SubList) :- S1 is Start - 1,
                                             E1 is End - 1,
                                             extract(Tail, S1, E1, SubList).
...
factorial2(0, 1).
factorial2(N,Sol) :-  factorial2(N1, Sol1),
                      N is N1 + 1,
                      Sol is Sol1 * N.
...
sublista([Head | Tail1], [Head | Tail2]) :- sublista(Tail1, Tail2).
...
descendantOf(X, Y) :- arc(N, X), descendantOf(N, Y).
...
descendants(N, Sol) :- arc(N, M), \+ (arc(N, P), M \= P),
                       descendants(M, Sol1), Sol is Sol1 + 1.
descendants(N, Sol) :- arc(N, M), (arc(N, P), M \= P),
                       descendants(M, Sol1),
                       descendants(P, Sol2),
                       Sol is Sol1 + Sol2 + 2.
