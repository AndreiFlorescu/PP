...
test1 = testOne 1 $ testVal (take 42 $ myCycle xs) (take 42 $ cycle xs) "myCycle" 1
  where xs = [1,2,3,4]
myCycle :: [a] -> [a]
myCycle = map head . iterate (\ list -> tail list ++ [head list])
...
arithmetic i r = iterate (+ r) i
geometric :: Num a => a -> a -> [a]
geometric i q = iterate (* q) i
{-
  3. (2p)
...
wordToAlpha = (\c -> chr $ ord 'a' + (\x -> mod x 26) (ord $ wordToChar c))
{-
    5. (1p)
...
tableToFunc table c = if x == c then y
                      else tableToFunc (tail table) c
                    where (x, y) = head table
        
substCrypt :: [(Char, Char)] -> String -> String
substCrypt table s = map (\c -> tableToFunc table c) s
{-
    7. (Bonus - 1p)
