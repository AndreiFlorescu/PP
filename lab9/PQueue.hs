...
class (Ord a) => PQueue pq a where
    ...
    toList x
      | isEmpty x = []
      | otherwise = (fromJust $ top x) : (toList $ pop x)
...
instance (Ord a) => PQueue ListPQ a where
    ...
    isEmpty (LPQ x) = null x

    insert el (LPQ x)
      | isEmpty (LPQ x) = LPQ [el]
      | fst el >= fst (head x) = LPQ $ el:x
      | otherwise = LPQ $ y : toList (insert el (LPQ ys))
        where
          y = head x
          ys = tail x

    top (LPQ x)
      | isEmpty (LPQ x) = Nothing
      | otherwise = Just $ head x

    pop (LPQ x) = LPQ $ tail x
...
merge (Node rank1 val1 l1 r1) (Node rank2 val2 l2 r2)
  | fst val1 <= fst val2 = merge (Node rank2 val2 l2 r2) (Node rank1 val1 l1 r1)
  | rank l1 < rank merged = (Node (rank l1 + 1) val1 merged l1)
  | otherwise = (Node (rank merged + 1) val1 l1 merged)
    where 
      merged = merge r1 (Node rank2 val2 l2 r2)

inorder :: LeftistPQ a -> [(Prio, a)]
inorder (Empty _) = []
inorder (Node rank val l r) = inorder l ++ [val] ++ inorder r
...
instance (Ord a) => PQueue LeftistPQ a where
    ...
    insert elem node = merge (Node 1 elem empty empty) node 

    top node
      | isEmpty node = Nothing
      | otherwise = Just $ nodeVal node

    pop x = merge (left x) (right x) 
