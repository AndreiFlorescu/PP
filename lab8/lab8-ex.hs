...
normalizeV (V a1 a2 a3) = (V (a1 / norm) (a2 / norm) (a3 / norm))
                            where norm = lengthV (V a1 a2 a3)
...
orthogonalV (v : []) = True
orthogonalV (v : vs) = if all (== 0) (map (dotV v) vs) == True
                          then orthogonalV vs
                          else False
...
deepEqual (List xs) (List ys) = if length xs == length ys
                                  then and $ zipWith deepEqual xs ys
                                  else False
...
insertElem (BSTNode cur left right) x = 
  if x <= cur
    then BSTNode cur (insertElem left x) right
    else BSTNode cur left (insertElem right x) 

findElem :: (Ord a, Eq a) => BST a -> a -> Maybe a
findElem BSTNil x = Nothing
findElem (BSTNode cur left right) x
  | x == cur = Just x
  | x < cur = findElem left x 
  | x > cur = findElem right x

subTree :: (Ord a, Eq a) => BST a -> a -> a -> Maybe (BST a)
subTree (BSTNode cur left right) x y
  | foundX == Nothing || foundY == Nothing = Nothing
  | x <= cur && y <= cur = subTree left x y
  | x > cur && y > cur = subTree left x y
  | otherwise = Just (BSTNode cur left right)
    where
      foundX = findElem (BSTNode cur left right) x
      foundY = findElem (BSTNode cur left right) y

inorder :: BST a -> [a]
inorder BSTNil = []
inorder (BSTNode cur left right) = inorder left ++ [cur] ++ inorder right
