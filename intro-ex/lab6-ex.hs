...
unzip2  :: [(a, b)] -> ([a], [b])
unzip2 [] = ([], [])
unzip2 ((a, b): rest) = (a : fst(unzip2 rest), b : snd(unzip2 rest))
...
-}
primes :: Int -> [Int]
primes n = [x | x <- [2..n], [y | y <- [2..x - 1], x `mod` y == 0] == []]
...
setUnion :: Eq a => [a] -> [a] -> [a]
setUnion a b = (setDiff (a ++ b) (setIntersection a b)) ++ (setIntersection a b)
...
group2 [] = []
group2 (x:[]) = [[x]]
group2 (x:y:end) =  let (first:rest) = group2 (y:end)
                    in if x == y
                      then (x:first):rest
                      else [x]:first:rest
...
-}
dup :: String -> String
dup input = unwords [unwords [x, x] | x <- words input]
