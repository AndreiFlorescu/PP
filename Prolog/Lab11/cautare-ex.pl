...
% S1.
next_state(misionari, CurState, NextState) :-
        parseState(CurState, M, NME ,NCE, NMV, NCV),
        M == est,
        boat(NM, NC),
        NM =< NME, NC =< NCE,
        NME_New is NME - NM, NCE_New is NCE - NC,
        NMV_New is NMV + NM, NCV_New is NCV + NC,
        safeMisionari(NME_New, NCE_New), safeMisionari(NMV_New, NCV_New),
        NextState = state(vest, NME_New, NCE_New, NMV_New, NCV_New).
...
next_state(misionari, CurState, NextState) :-
        ...
        NMV_New is NMV - NM, NCV_New is NCV - NC,
        safeMisionari(NME_New, NCE_New), safeMisionari(NMV_New, NCV_New),
        NextState = state(est, NME_New, NCE_New, NMV_New, NCV_New).
...
preorder(N, Parc) :- dfs(N, Parc).

dfs(N, Sol) :-
        dfs([N], [], Order),
        reverse(Order, Sol).

dfs([], Vis, Order) :- Order = Vis.

dfs([H | T], Vis, Order) :-
        \+ arc(H, _), dfs(T, [H | Vis], Order).

dfs([H | T], Vis, Order) :-
        arc(H, Kids),
        append(Kids, T, NewT),
        dfs(NewT, [H | Vis], Order).
